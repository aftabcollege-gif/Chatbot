...
export function Switch({ className, ...props }: React.ComponentPropsWithoutRef<typeof SwitchPrimitive.Root>) {
  return (
    <SwitchPrimitive.Root
      ...
      {...props}
    >
      <SwitchPrimitive.Thumb className="pointer-events-none block h-4 w-4 rounded-full bg-background shadow-lg ring-0 transition-transform data-[state=checked]:translate-x-[-16px] data-[state=unchecked]:translate-x-0 rtl:data-[state=checked]:translate-x-[-16px]" />
    </SwitchPrimitive.Root>
  );
}
