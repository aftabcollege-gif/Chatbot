...
_PERSIAN_MAP = str.maketrans(
    {
        ...
        "\u0656": "",
        "\u0670": "",
        # ZWNJ (\u200c) must be PRESERVED in Persian. Words like می‌توانم,
        # خانه‌ها, و... depend on it. We used to replace it with a space which
        # broke tokenization and made Persian output look "broken". Note: for
        # FTS we still search on the normalized (de-ZWNJ) variant through the
        # content_normalized column; user-facing text keeps ZWNJ.
        "\u200f": "",
        "\u200e": "",
        "\xa0": " ",
    }
...
def normalize_persian(text: str, *, remove_zwnj: bool = False) -> str:
    """Normalize Persian text for indexing/searching.

    When ``remove_zwnj`` is True, ZWNJ is replaced with space (used only for
    generating the normalized column that FTS/vector search see; the
    user-facing ``content`` column always preserves ZWNJ so Persian output
    looks correct).
    """
    if not text:
        return ""
...
def normalize_light(text: str) -> str:
    """Lighter normalization for display-friendly comparisons (preserves ZWNJ)."""
    if not text:
        return ""
    text = unicodedata.normalize("NFKC", text)
    text = text.translate(_DIGIT_MAP)
